...
class ScheduleAddClientOrder2LiveUpdateController extends Controller
{
    public function liveupdate(Request $request)
    {
        ...
        $new_order->locationID = 1;
        $new_order->remark = 'N/A';
        $new_order->dateDelivered = null;
        $new_order->schedType = "Manufacturer";
        $new_order->contactPerson = $request->contactPerson;
        $new_order->save();

        foreach($request->orders as $order){
            $new_order_detail = new ScheduleManufacturerDetail();
            $new_order_detail->supplyID= $order[1];
